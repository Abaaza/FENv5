...
$EC2_PUBLIC_IP = "54.90.3.22"              # Replace with your EC2 IP (e.g., "54.123.456.789")
...
# Validate inputs
$errors = @()
if ($EC2_PUBLIC_IP -eq "54.90.3.22") {
    $errors += "âŒ EC2_PUBLIC_IP not set - please add your EC2 IP address"
}
if ($PEM_FILE_PATH -eq "C:\\Users\\abaza\\Downloads\\tfp-boq-key.pem") {
    $errors += "âŒ PEM_FILE_PATH not set - please add path to your .pem key file"
}
if ($AMPLIFY_URL -eq "YOUR-AMPLIFY-URL-HERE") {
    $errors += "âŒ AMPLIFY_URL not set - please add your Amplify URL"
}
if (!(Test-Path $PEM_FILE_PATH) -and $PEM_FILE_PATH -ne "C:\\Users\\abaza\\Downloads\\tfp-boq-key.pem") {
    $errors += "âŒ PEM file not found at: $PEM_FILE_PATH"
}
...
Write-Host ""
Write-Host "ðŸ“ EC2 IP: $EC2_PUBLIC_IP" -ForegroundColor White
Write-Host "ðŸŒ Amplify URL: $AMPLIFY_URL" -ForegroundColor White
Write-Host "ðŸ”‘ PEM File: $PEM_FILE_PATH" -ForegroundColor White
Write-Host ""
...
try {
    ...
    if (!(Test-Path $deployScript)) {
        Write-Host "`nâŒ Error: deploy-to-ec2-automated.ps1 not found!" -ForegroundColor Red
    ...
    if ($V2_API_KEY -ne "your-v2-api-key-here" -or $V1_API_KEY -ne "your-v1-api-key-here") {
        Write-Host "`nðŸ”‘ Updating API keys..." -ForegroundColor Yellow
...
"@
    ...
    Write-Host "â•‘         ðŸŽ‰ DEPLOYMENT COMPLETED SUCCESSFULLY! ðŸŽ‰        â•‘" -ForegroundColor Green
    Write-Host "â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•" -ForegroundColor Green
    Write-Host ""
    Write-Host "Your backend is now live at:" -ForegroundColor Cyan
    Write-Host "ðŸ‘‰ https://$EC2_PUBLIC_IP/api" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "`nâŒ Deployment failed with error:" -ForegroundColor Red
